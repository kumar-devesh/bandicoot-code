...
struct Base_inv_yes
  {
  coot_inline const Op<eT, derived, op_inv> i() const;   //!< matrix inverse
  };
...
struct Base_trans_cx
  {
  coot_inline const Op<eT, derived, op_htrans>  t() const;
  coot_inline const Op<eT, derived, op_htrans> ht() const;
  coot_inline const Op<eT, derived, op_strans> st() const;  // simple transpose: no complex conjugates
  };
...
struct Base_trans_default
  ...
  coot_inline const Op<eT, derived, op_htrans>  t() const;
  coot_inline const Op<eT, derived, op_htrans> ht() const;
  coot_inline const Op<eT, derived, op_htrans> st() const;  // return op_htrans instead of op_strans, as it's handled better by matrix multiplication code
  };
