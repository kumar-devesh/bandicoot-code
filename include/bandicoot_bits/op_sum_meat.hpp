...
op_sum::apply_noalias(Mat<eT>& out, const Mat<eT>& A, const uword dim)
  ...
  if(dim == 0)
    ...
    opencl::runtime_t::adapt_uword A_n_rows(A.n_rows);
    opencl::runtime_t::adapt_uword A_n_cols(A.n_cols);
...
op_sum::apply_noalias(Mat<eT>& out, const subview<eT>& sv, const uword dim)
  ...
  if(dim == 0)
    ...
    opencl::runtime_t::adapt_uword sv_m_n_rows(sv.m.n_rows);
    
    opencl::runtime_t::adapt_uword start_row(sv.aux_row1);
    opencl::runtime_t::adapt_uword start_col(sv.aux_col1);
    
    opencl::runtime_t::adapt_uword sub_n_rows(sv.n_rows);
    opencl::runtime_t::adapt_uword sub_n_cols(sv.n_cols);
