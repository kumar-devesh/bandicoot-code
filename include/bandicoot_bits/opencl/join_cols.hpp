...
join_cols(dev_mem_t<eT5> out, const dev_mem_t<eT1> A, const uword A_n_rows, const uword A_n_cols, const dev_mem_t<eT2> B, const uword B_n_rows, const uword B_n_cols, const dev_mem_t<eT3> C, const uword C_n_rows, const uword C_n_cols, const dev_mem_t<eT4> D, const uword D_n_rows, const uword D_n_cols)
  ...
  if (A_n_elem > 0)
    {
    inplace_op_subview(out, A, out_n_rows, 0, 0, A_n_rows, A_n_cols, twoway_kernel_id::submat_inplace_set_mat, "coot::opencl::join_cols()");
    }

  if (B_n_elem > 0)
    {
    inplace_op_subview(out, B, out_n_rows, A_n_rows, 0, B_n_rows, B_n_cols, twoway_kernel_id::submat_inplace_set_mat, "coot::opencl::join_cols()");
    }

  if (C_n_elem > 0)
    {
    inplace_op_subview(out, C, out_n_rows, (A_n_rows + B_n_rows), 0, C_n_rows, C_n_cols, twoway_kernel_id::submat_inplace_set_mat, "coot::opencl::join_cols()");
    }

  if (D_n_elem > 0)
    {
    inplace_op_subview(out, D, out_n_rows, (A_n_rows + B_n_rows + C_n_rows), 0, D_n_rows, D_n_cols, twoway_kernel_id::submat_inplace_set_mat, "coot::opencl::join_cols()");
    }
  }
