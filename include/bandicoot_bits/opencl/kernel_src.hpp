...
kernel_src::init_oneway_source()
  ...
  "  } \n"
/*
  "__kernel void COOT_FN(PREFIX,accu_interleaved)(__global const eT1* in_mem, const UWORD n_elem, __global eT1* out_mem, __local eT1* aux_mem) \n"
  "  { \n"
  "  const UWORD tid = get_global_id(0); \n"
  "  UWORD i = get_global_id(1) * (get_global_size(0) * 2) + tid; \n"
  "  const UWORD grid_size = get_global_size(0) * 2 * get_global_size(1); \n"
  "  \n"
  "  aux_mem[tid] = 0; \n"
  "  if (get_global_size(0) < WAVEFRONT_SIZE) \n"
  "    { \n"
  "    UWORD tid_next = tid + get_global_size(0); \n"
  "    while (tid_next < WAVEFRONT_SIZE) \n"
  "      { \n"
  "      aux_mem[tid_next] = 0; \n"
  "      tid_next += get_global_size(0); \n"
  "      } \n"
  "    barrier(CLK_LOCAL_MEM_FENCE); \n"
  "    } \n"
  "  \n"
  ...
  "  barrier(CLK_LOCAL_MEM_FENCE); \n"
  "  \n"
  "  for (UWORD s = get_global_size(0) / 2; s > WAVEFRONT_SIZE; s >>= 1) \n"
  "    { \n"
  "    if (tid < s) \n"
  "      { \n"
  "      aux_mem[tid] += aux_mem[tid + s]; \n"
  ...
  "    { \n"
  "    COOT_FN_3(PREFIX,accu_wavefront_reduce_,WAVEFRONT_SIZE_NAME)(aux_mem, tid); \n"
//  "    COOT_FN(PREFIX,accu_wavefront_reduce_32)(aux_mem, tid);\n"
  "    } \n"
  "  \n"
  "  if (tid == 0) \n"
  "    { \n"
  "    out_mem[get_global_id(1)] = aux_mem[0]; \n"
  "    } \n"
  "  } \n"
  "\n"
*/
