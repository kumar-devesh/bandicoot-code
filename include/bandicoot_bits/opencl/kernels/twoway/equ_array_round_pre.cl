...
__kernel
void
COOT_FN(PREFIX,equ_array_round_pre)(__global eT2* dest,
  ...
  if (row < n_rows && col < n_cols)
    ...
    if (COOT_FN(coot_is_fp_,eT2)())
      {
      const fp_eT2 val = (fp_eT2) src[src_index];
      dest[dest_index] = (eT2) round(val);
      }
    else
      {
      dest[dest_index] = (eT2) src[src_index];
      }
    }
  }
