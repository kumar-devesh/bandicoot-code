...
sort_index_vec(dev_mem_t<uword> out, dev_mem_t<eT> A, const uword n_elem, const uword sort_type, const uword stable_sort)
  ...
  status |= clSetKernelArg(k, 1, sizeof(cl_mem), &(out.cl_mem_ptr));
  status |= clSetKernelArg(k, 2, sizeof(cl_mem), &(tmp_mem.cl_mem_ptr));
  status |= clSetKernelArg(k, 3, sizeof(cl_mem), &(tmp_mem_index.cl_mem_ptr));
  status |= clSetKernelArg(k, 4, cl_n_elem.size, cl_n_elem.addr);
  status |= clSetKernelArg(k, 5, aux_mem_size,   NULL);

  coot_check_cl_error(status, "coot::opencl::radix_sort_index(): failed to set kernel arguments");
  ...
  coot_check_cl_error(status, "coot::opencl::radix_sort_index(): failed to run kernel");
