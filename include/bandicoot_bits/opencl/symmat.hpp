...
symmat(dev_mem_t<eT2> out, const dev_mem_t<eT1> in, const uword size, const uword lower)
  ...
  coot_debug_check( (get_rt().cl_rt.is_valid() == false), "coot::cuda::symmat(): OpenCL runtime not valid");
  ...
  if (out.cl_mem_ptr == in.cl_mem_ptr)
    ...
    status |= clSetKernelArg(kernel, 1, cl_size.size,   cl_size.addr);
    }
  else
    {
    ...
    status |= clSetKernelArg(kernel, 2, cl_size.size,   cl_size.addr);
    }

  coot_check_cl_error(result, "coot::opencl::symmat(): could not set arguments for symmat kernel");
  ...
  coot_check_cuda_error(result, "coot::opencl::symmat(): couldn't execute kernel");
  }
