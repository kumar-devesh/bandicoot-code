...
Mat<eT>::get_submatrix(Mat<eT>& X, const uword start_row, const uword start_col, const uword end_row, const uword end_col)
  ...
  cl_int status = clEnqueueCopyBufferRect(get_rt().cl_rt.get_cq(), (*this).get_dev_mem(false), X.get_dev_mem(false), src_origin, dst_origin, region, src_row_pitch, src_slice_pitch, dst_row_pitch, dst_slice_pitch, 0, NULL, NULL);
...
Mat<eT>::set_submatrix(Mat<eT>& X, const uword start_row, const uword start_col)
  ...
  cl_int status = clEnqueueCopyBufferRect(get_rt().cl_rt.get_cq(), X.get_dev_mem(false), (*this).get_dev_mem(false), src_origin, dst_origin, region, src_row_pitch, src_slice_pitch, dst_row_pitch, dst_slice_pitch, 0, NULL, NULL);
...
Mat<eT>::get_sum_all_1(Mat<eT>& X)
  ...
  cl_kernel kernel = get_rt().cl_rt.get_kernel<float>(kernel_id::sum_all);
  ...
  status = clEnqueueNDRangeKernel(get_rt().cl_rt.get_cq(), kernel, work_dim, global_work_offset, global_work_size, local_work_size, 0, NULL, NULL);
...
Mat<eT>::get_sum_all_3(Mat<eT>& X)
  ...
  const cl_uint n_units = get_rt().cl_rt.get_n_compunits();
...
Mat<eT>::get_sum_colwise(Mat<eT>& X)
  ...
  cl_kernel kernel = get_rt().cl_rt.get_kernel<float>(kernel_id::sum_colwise);
...
Mat<eT>::get_sum_rowwise(Mat<eT>& X)
  ...
  cl_kernel kernel = get_rt().cl_rt.get_kernel<float>(kernel_id::sum_rowwise);
...
Mat<eT>::get_sum_submat_colwise(Mat<eT>& X, const uword start_row, const uword start_col, const uword end_row, const uword end_col)
  ...
  cl_kernel kernel = get_rt().cl_rt.get_kernel<float>(kernel_id::submat_sum_colwise);
...
Mat<eT>::get_sum_submat_rowwise(Mat<eT>& X, const uword start_row, const uword start_col, const uword end_row, const uword end_col)
  ...
  cl_kernel kernel = get_rt().cl_rt.get_kernel<float>(kernel_id::submat_sum_rowwise);
