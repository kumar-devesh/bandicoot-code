...
class glue_solve
  ...
  template<typename out_eT, typename T1, typename T2>
  inline static std::tuple<bool, std::string> apply(Mat<out_eT>& out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const uword flags);
...
namespace solve_opts
  ...
  struct opts_fast         : public opts { inline constexpr opts_none()         : opts(flag_fast        ) {} };
