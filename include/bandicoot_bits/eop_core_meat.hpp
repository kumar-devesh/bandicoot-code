...
eop_core<eop_type>::apply(Mat<typename T1::elem_type>& out, const eOp<T1, eop_type>& x)
  ...
  cl_kernel kernel;
       ...
       if(is_same_type<eop_type, eop_scalar_plus      >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_plus_scalar      ); }
  else if(is_same_type<eop_type, eop_neg              >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_neg              ); }
  else if(is_same_type<eop_type, eop_scalar_minus_pre >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_minus_scalar_pre ); }
  else if(is_same_type<eop_type, eop_scalar_minus_post>::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_minus_scalar_post); }
  else if(is_same_type<eop_type, eop_scalar_times     >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_mul_scalar       ); }
  else if(is_same_type<eop_type, eop_scalar_div_pre   >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_div_scalar_pre   ); }
  else if(is_same_type<eop_type, eop_scalar_div_post  >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_div_scalar_post  ); }
  else if(is_same_type<eop_type, eop_square           >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_square           ); }
  else if(is_same_type<eop_type, eop_sqrt             >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_sqrt             ); }
  else if(is_same_type<eop_type, eop_exp              >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_exp              ); }
  else if(is_same_type<eop_type, eop_log              >::yes)  { kernel = coot_rt.cl_rt.get_kernel<eT>(kernel_id::equ_array_log              ); }
  ...
  status |= clEnqueueNDRangeKernel(coot_rt.cl_rt.get_cq(), kernel, 1, NULL, &work_size, NULL, 0, NULL, NULL);
