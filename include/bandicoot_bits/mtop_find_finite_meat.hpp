...
inline
void
mtop_find_finite::apply(Mat<uword>& out, const mtOp<uword, T1, mtop_find_finite>& in)
  {
  ...
  if (!is_real<eT>::value)
    {
    // If the type is not real, then all elements are finite, and thus we can take a shortcut to return.
    out = linspace<uvec>(0, E.M.n_elem - 1, E.M.n_elem);
    return;
    }

  // For the first step, we have to find all finite values.
  Mat<uword> X(E.M.n_rows, E.M.n_cols);
  coot_rt_t::relational_unary_array_op(X.get_dev_mem(false), E.M.get_dev_mem(false), E.M.n_elem, oneway_real_kernel_id::rel_isfinite, "find_finite");
...
inline
uword
mtop_find_finite::compute_n_rows(const mtOp<out_eT, T1, mtop_find_finite>& op, const uword in_n_rows, const uword in_n_cols)
  {
...
inline
uword
mtop_find_finite::compute_n_cols(const mtOp<out_eT, T1, mtop_find_finite>& op, const uword in_n_rows, const uword in_n_cols)
  {
  return 1;
  }
