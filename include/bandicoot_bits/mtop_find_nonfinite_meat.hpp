...
mtop_find_nonfinite::apply(Mat<uword>& out, const mtOp<uword, T1, mtop_find_nonfinite>& in)
  ...
  if (!is_real<eT>::value)
    {
    // If the type is not a real type, then all elements must be finite.
    out.reset();
    return;
    }
  ...
  coot_rt_t::relational_unary_array_op(X.get_dev_mem(false), E.M.get_dev_mem(false), E.M.n_elem, oneway_real_kernel_id::rel_isnonfinite, "find_nonfinite");
...
inline
uword
mtop_find_nonfinite::compute_n_rows(const mtOp<out_eT, T1, mtop_find_nonfinite>& op, const uword in_n_rows, const uword in_n_cols)
  {
...
inline
uword
mtop_find_nonfinite::compute_n_cols(const mtOp<out_eT, T1, mtop_find_nonfinite>& op, const uword in_n_rows, const uword in_n_cols)
  {
  return 1;
  }
