...
__global__
void
COOT_FN(PREFIX,sqrd_dbl_norm2_small)(const eT1* in_mem,
                                     const UWORD n_elem,
                                     double* out_mem)
  {
  ...
  while (i + blockDim.x < n_elem)
    ...
    const double v1 = (double) in_mem[i];
    const double v2 = (double) in_mem[i + blockDim.x];
    aux_mem[tid] += (v1 * v1) + (v2 * v2);
    i += grid_size;
    }
  if (i < n_elem)
    {
    const double v = (double) in_mem[i];
    aux_mem[tid] += (v * v);
    }
