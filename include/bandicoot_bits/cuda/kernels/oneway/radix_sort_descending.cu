...
__global__
void
COOT_FN(PREFIX,radix_sort_descending)(eT1* A,
                                      eT1* tmp_mem,
                                      const UWORD n_elem)
  {
  ...
  for (UWORD b = 0; b < max_bit; ++b)
    ...
    // Since we want the largest values to come first, the first entry will be the count of 1s.
    ...
    //    aux_mem[tid] should hold the first place to put a 1 point
    //    aux_mem[tid + num_threads] should hold the first place to put a 0 point
    // More specifically:
  ...
  // In both cases, we have to put the 0-bit values before the 1-bit values (since we are sorting in descending order).
  ...
  // perform a prefix sum, as with the rest of the bits; positive points will come first
  ...
  if (coot_is_fp((eT1) 0))
    ...
    // Here, we have values in the right order, we just need to put the positive values ahead of the negative values.
