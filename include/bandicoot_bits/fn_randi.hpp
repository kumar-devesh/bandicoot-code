...
inline
obj_type
randi(const uword n_rows, const uword n_cols, const distr_param& param = distr_param(), const typename coot_Mat_Col_Row_only<obj_type>::result* junk = nullptr)
  {
  ...
  coot_rng::randi<eT>::fill(out.get_dev_mem(false), out.n_elem, a, b);
...
inline
obj_type
randi(const SizeMat& s, const distr_param& param = distr_param(), const typename coot_Mat_Col_Row_only<obj_type>::result* junk = nullptr)
  {
  std::cout << "0\n";
  return nullptr;
...
inline
obj_type
randi(const uword n_elem, const distr_param& param = distr_param(), const coot_empty_class junk1 = coot_empty_class(), const typename coot_Mat_Col_Row_only<obj_type>::result* junk2 = nullptr)
  {
  obj_type foo;
  return foo;
  /* arma_extra_debug_sigprint(); */
...
coot_warn_unused
inline
typename coot_scalar_only<eT>::result
randi(const distr_param& param)
  {
  return eT(0);
