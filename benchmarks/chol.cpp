...
void fill_randu(MatType& x)
  ...
  // But it needs to be positive definite...
  cpu_x = cpu_x.t() * cpu_x;
  cpu_x.diag() += 3;
...
template<typename MatType>
double run_benchmark(const uword rows, const bool cuda)
  {
  ...
  c.tic();
  const bool status = chol(r, x);
  get_rt().synchronise();
  t = c.toc();

  if (!status)
    std::cerr << "chol() failed!\n";
...
void run_benchmarks(const uword rows,
  ...
  for (size_t trial = 0; trial < trials; ++trial)
    {
    const double t = run_benchmark<MatType>(rows, cuda);
...
int main(int argc, char** argv)
  ...
  std::cout << "chol: Cholesky decomposition benchmark comparison\n";
  ...
  run_benchmarks<arma::fmat>(rows, false, trials, "chol", device_name, "cpu", "float", out_file);
  run_benchmarks<coot::fmat>(rows, false, trials, "chol", device_name, "opencl", "float", out_file);
  run_benchmarks<coot::fmat>(rows, true, trials, "chol", device_name, "cuda", "float", out_file);

  run_benchmarks<arma::mat>(rows, false, trials, "chol", device_name, "cpu", "double", out_file);
  run_benchmarks<coot::mat>(rows, false, trials, "chol", device_name, "opencl", "double", out_file);
  run_benchmarks<coot::mat>(rows, true, trials, "chol", device_name, "cuda", "double", out_file);
  }
