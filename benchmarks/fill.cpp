/**
 * Benchmark comparison of fill() between CUDA and OpenCL backends.
 */
#include <bandicoot>
#include <armadillo>
...
double run_benchmark(const uword elem, const bool cuda)
  ...
  c.tic();
  x.fill(typename MatType::elem_type(0));
  get_rt().synchronise();
  t = c.toc();
...
int main(int argc, char** argv)
  ...
  run_benchmarks<arma::fmat>(elem, false, trials, "fill", device_name, "cpu", "float", out_file);
  run_benchmarks<coot::fmat>(elem, false, trials, "fill", device_name, "opencl", "float", out_file);
  run_benchmarks<coot::fmat>(elem, true, trials, "fill", device_name, "cuda", "float", out_file);

  run_benchmarks<arma::mat>(elem, false, trials, "fill", device_name, "cpu", "double", out_file);
  run_benchmarks<coot::mat>(elem, false, trials, "fill", device_name, "opencl", "double", out_file);
  run_benchmarks<coot::mat>(elem, true, trials, "fill", device_name, "cuda", "double", out_file);
  }
