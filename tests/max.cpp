...
TEMPLATE_TEST_CASE("two_subview_max", "[max]", float, double, u32, s32, u64, s64)
  {
  typedef TestType eT;
  ...
  Mat<eT> z = max(x.submat(50, 50, 74, 74), y.submat(25, 25, 49, 49));

  Mat<eT> x_extracted = x.submat(50, 50, 74, 74);
  Mat<eT> y_extracted = y.submat(25, 25, 49, 49);

  Mat<eT> z_ref = max(x_extracted, y_extracted);
